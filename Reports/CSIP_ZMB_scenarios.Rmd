...
Figure 1 depicts  historical trends and projections for GDP and population for the BAU scenario as well as two other SSP scenarios to assess uncertainties in future trends (see below). The scenarios describe the period from present to 2050, which is commonly used as endpoint in many climate change and population projections. We also include the historical trend. Assumptions on GDP growth and population growth for the rest of the world are presented in Figure A1 in the Annex. 

_Figure 1: GDP and Population projections_
...
Finally, to model the BAU scenario, we also make assumptions on area of protected land, change in diets, and food loss and waste at the global level. Table A1 in the Annex, presents details on how these scenario assumptions and how they are modelled in GLOBIOM. 


# Normative scenario
The normative scenario represents desirable future agricultural sector. The scenario is based on the input that was provided at a stakeholder workshop as well as several national and agricultural planning reports that have been published over the last decade by the Zambian government.[1^] The scenario will be used as a benchmark to assess  to what extent the implementation of various policy options, will bring the agricultural sector closer to the desired state. Table 1 presents key indicator values for the normative scenario as well as the source on which it is based. In a next step, the variables will be extrapolated to 2050. 
...
# Policy scenarios
At the stakeholder workshop, several climate smart agricultural policy options were discussed and ranked by the participants. Policy options covered a large number of areas. The highest ranked was "crop & livestock varieties". Table 2 presents a selection of the policy options that will be included in the scenario analysis. It also presents information on how we aim to model the policy in GLOBIOM. Some proposed policy options cannot be included in the analysis because they refer to markets are elements of the agricultural sector that are not modelled by GLOBIOM, e.g. "At least 60% of men, women and youth involved in agriculture belong to functioning savings groups".
