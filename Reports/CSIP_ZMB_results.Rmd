...
Figure X shows the nett agricultural trade (exports minus imports) for a number of key agricultural commodities for 2010 and 2050 as well as the vision target of doubling (nett) exports in 2050. The results are mixed. For cotton, millet and maize, nett trade is positive in 2010 and an increase is expected for 2050. Only in case of millet, the target of doubling net trade will be achieved. For cassava and sugarcane, which also exhibit a positive nett trade in both 2010 and 2050, a decrease is projeced. Finally, for bovine meat, groundnuts, pig meat, rice, soya, sweet potatoes and wheat, net imports (negative net trade) are expected to increase even further. The error bars indicate that climate change can have substantial impact on the trade balance of several crops, in particular cassava, maize, cotton, groundnuts and sugarcane. In the latter case, in one extreme climate scenario nett trade is projected to be higher than the 2010 value, almost reaching the vision target, while in the other extreme scenario, net trade will become negative.  
...
Under all CSA strategy scenarios (not considering dtm) the cropland area is lower than the BAU (Figure X). However, with a reduction of between 1-5%, the overall impact of adoption is relatively low. The, albeit small, expansion of crop land has a positive effect on natural land use, of which 1-2% is now saved in comparison to the BAU. The pattern of forest and grassland expansion is not noticably different from the BAU.

The land use change projections are very sensitive to climate change. This is shown by the error bars, which for nearly all land use classes spans both negative and positive values. For example, depending on the climate change scenario and CSA option, cropland change can differ between +2 and -1% in comparison to the BAU in the msd scenario. In the wors case if cropland would expand with 2%, total crop land exceed the cropland target as stated in the vision but only with a very small margin.
...
Figure X shows the change in kcal/cap/day, a proxy of food availability, in comparison with the BAU in the year 2050. Agro-forestry and phl (both 5%) are the two CSA strategies that have the highest impact on food availability. The total effect is smaller than the increase in agricultural production (Figure X) because not all production is for food consumption. 
...
Figure X shows that expansion of ca, msd and phl practices results in decrease  of emissions from fertilizer use with 7%, 9% and 7%, respectively in comparison to the BAU. The impact of phl is indirect as [@ADD ADD]. In contrast with the other options, the expansion of rr will lead to an increase in GHG emissions with no less than 29 percent. This clearly illustrates that there is a trade off between increasing productivity and decreasing emmissions, when rr is consired. 

If we only consider emissions from synthetic fertilizer use, the stand-alone adoption of ca, msd and phl practices by farmers in Zambia will not be sufficient to reach the INDC targets for reducing GHG emissions by 25-47%. 
